...
class Board():
    ...
    def check_move(self, s, t, player):
        ...
        if self.state[t] != '.': # occupied place
            valid_flag = False

        if sorted([s,t]) not in self.edges: # not adjacent
            valid_flag = False
    ...
    def check_remove(self, pos, player):
        ...
        if self.state[pos] == '.': # remove an empty place
            valid_flag = False

        if self.state[pos] == player.get_symbol(): # remove the men of the player self
            valid_flag = False
    ...
    def form_mill(self, pos, player):
        ...
        ### TODO
        player_sym = player.get_symbol()
        new_player_poses = []
        for idx in range(len(self.state)):
            if self.state[idx] == player_sym:
                new_player_poses.append(idx)

        for mill_sample in self.mills:
            if pos in mill_sample:
                if all(posx in new_player_poses for posx in mill_sample):
                    if_form = True
                    break
    ...
    def check_win(self, player, opponent):
        ...
        oppo_sym = opponent.get_symbol()

        for idx in range(len(self.state)):
            if self.state[idx] == oppo_sym:
                num_pieces += 1
            ...
            for i in range(len(self.state)):
                if self.state[i] == opponent.get_symbol():
                    ...
                    for j, k in self.edges:
                        ...
                        if j == i and self.state[k] == '.':
                            piece_can_move = True
                            break
                        if k == i and self.state[j] == '.':
                            piece_can_move = True
                            break
